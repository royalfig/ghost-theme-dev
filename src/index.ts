...
async function init() {
  ...
  if (argv.includes("--help") || argv.includes("-h") || !command) {
    console.log(chalk.bold(`\n  gtb - Ghost Theme Builder v3.0.0`));
    console.log(`\n  ${chalk.bold("USAGE")}`);
    console.log(`    gtb <command> [options]`);
    
    console.log(`\n  ${chalk.bold("COMMANDS")}`);
    console.log(`    dev          Start development server with live reload (default)`);
    console.log(`    init         Scaffold a new theme and link to local Ghost`);
    ...
    console.log(`    clone        Clone content from remote Ghost to local instance`);
  ...
  if (command === "init" || argv.includes("--init")) {
    ...
    if (linked) {
        console.log(chalk.green("⬥"), "  Theme skeleton created and linked successfully.");
    } else {
        console.log(chalk.green("⬥"), "  Theme skeleton created successfully (linking skipped or failed).");
    }
    return;
  }
  ...
  const res = await writeAssets([...jsEntryPoints, ...cssEntryPoints], port, isWatch, config);

  if (isWatch) {
    const url = await parseGhostCliOutput();
    const watcher = chokidar.watch(".", {
      ignored: [/(^|[\/\\])\../, "**/built/**", "dist/**", "node_modules"],
    });
