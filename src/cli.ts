...
export async function makeSkeleton() {
  ...
  if (kit === "tailwind") {
      filesToCheck.push(
          { filename: "assets/css/index.css", content: TAILWIND_V4_CSS },
          { filename: "tailwind.config.js", content: TAILWIND_CONFIG_TEMPLATE }
      );
  } else {
      filesToCheck.push(
  ...
  try {
    ...
    if (!existsSync(join(folderPath, ".git"))) {
        ...
        try {
            ...
            console.log(chalk.yellow("┃"), " Git initialization skipped (not found or error).");
        }
    }
...
export async function zipTheme() {
    const folderPath = process.cwd();
    const themeName = basename(folderPath);
    const pkg = JSON.parse(await readFilePromise(join(folderPath, "package.json"), "utf8") || "{}");
    ...
    output.on("close", () => {
        console.log(chalk.green("⬥"), ` Theme packaged to dist/${zipName} (${(archive.pointer() / 1024 / 1024).toFixed(2)} MB)`);
    });
...
export async function deployTheme() {
    ...
    if (!url || !key) {
        ...
        console.log(chalk.dim("┃"), " Set GHOST_ADMIN_API_URL and GHOST_ADMIN_API_KEY env variables.");
        return;
    }
...
export async function symLinkTheme(): Promise<boolean> {
    ...
    if (!hasCommand("ghost")) {
        ...
        const installCli = await confirm({ message: "Would you like to install Ghost CLI globally now?", default: true });
    ...
    if (existsSync(ghostConfigPath)) {
        try {
            ...
            if (config.instances) {
                instances = Object.entries(config.instances).map(([name, data]: [string, any]) => ({
                    name,
                    location: data.cwd
                }));
            }
    ...
    if (instances.length === 0) {
        ...
        const setupLocal = await confirm({ message: "Would you like to set up a local Ghost installation in the parent directory?", default: true });
        
        if (setupLocal) {
            const folderName = await input({ message: "What should we name the Ghost directory?", default: "ghost-local" });
    ...
    try {
        let targetInstance = instances.length === 1 ? instances[0] : await select({
            message: "Link to which Ghost instance?",
            choices: instances.map(i => ({ name: `${i.name} (${i.location})`, value: i }))
        });
        ...
        try {
            ...
            if (stats.isSymbolicLink()) {
                ...
                const replaceLink = await confirm({ message: `Already linked to ${existingLink}. Replace?`, default: true });
                if (replaceLink) {
                ...
                const replaceDir = await confirm({ message: "Directory already exists and is not a link. Replace with link?", default: false });
                if (replaceDir) {
        ...
        try {
            ...
            if (process.platform === 'win32') {
                console.log(chalk.yellow("┃"), " Note: On Windows, symlinks may require Administrator privileges or Developer Mode.");
            }
            return false;
        }
    } catch (e: any) { 
...
export async function cloneContent() {
    ...
    if (!url || !key) {
        console.error(chalk.red("✘  Missing Ghost Admin API credentials for the remote site."));
        console.log(chalk.dim("┃"), " Set GHOST_ADMIN_API_URL and GHOST_ADMIN_API_KEY env variables.");
        return;
    }
    ...
    const instances = rows.map(row => {
        const parts = row.split("│").map(p => p.trim());
        return { name: parts[1], location: parts[2], url: parts[5] };
    });

    if (instances.length === 0) {
        console.error(chalk.red("✘  No local Ghost instances found to clone into."));
        return;
    }
    ...
    const localContentPath = join(targetInstance.location.replace(/^~/, process.env.HOME || ""), "content");
    const api = new GhostAdminApi({ url, key, version: "v5.0" });

    console.log(chalk.blue("⬥"), ` Exporting content from ${url}...`);
    try {
        const exportData = await api.makeRequest({
            url: "/db/",
            method: "GET"
        });

        const exportFilePath = join(folderPath, `ghost-export-${Date.now()}.json`);
        await writeFilePromise(exportFilePath, JSON.stringify(exportData, null, 2));
        console.log(chalk.green("✔"), ` Content exported to ${basename(exportFilePath)}`);

        // Find all images in the export
        const exportString = JSON.stringify(exportData);
        // Match /content/images/ paths until a delimiter
        const imageMatches = exportString.match(/\/content\/images\/[^"'\s>]+/g) || [];
        const uniqueImages = [...new Set(imageMatches)];

        if (uniqueImages.length > 0) {
            console.log(chalk.blue("⬥"), ` Found ${uniqueImages.length} unique images. Downloading...`);
            let downloaded = 0;
            let skipped = 0;

            for (const imgPath of uniqueImages) {
                const remoteUrl = `${url.replace(/\/$/, "")}${imgPath}`;
                const localPath = join(localContentPath, "images", imgPath.replace("/content/images/", ""));
                ...
                process.stdout.write(`\r${chalk.dim("┃")} Progress: ${downloaded + skipped}/${uniqueImages.length} (Downloaded: ${downloaded}, Skipped: ${skipped})`);
            }
            console.log("\n" + chalk.green("✔"), " Image sync complete!");
        }

        console.log("\n" + chalk.bold.blue("⬥ Next Steps:"));
        console.log(chalk.white(` 1. Open your local Ghost Admin: ${targetInstance.url}/ghost/`));
        console.log(chalk.white(` 2. Go to Settings > Labs > Import content.`));
        console.log(chalk.white(` 3. Upload the exported file: ${basename(exportFilePath)}`));
        console.log(chalk.dim("\n Note: The local Ghost CLI doesn't reliably support programmatic imports via the experimental /db/ endpoint."));

    } catch (error: any) {
        console.error(chalk.red("✘  Cloning failed:"), error.message);
    }
}
