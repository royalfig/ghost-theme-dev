...
export async function cloneContent() {
  ...
  if (!url || !key) {
    console.error(
      chalk.red("✘  Missing Ghost Admin API credentials for the remote site."),
    );
    console.log(
      chalk.dim("┃"),
      " Set GHOST_ADMIN_API_URL and GHOST_ADMIN_API_KEY env variables.",
  ...
  if (instances.length === 0) {
    console.error(
      chalk.red("✘  No local Ghost instances found to clone into."),
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      chalk.green("✔"),
      ` Content exported to ${basename(exportFilePath)}`,
    );
    ...
    const imageMatches =
      exportString.match(/\/content\/images\/[^"'\s>]+/g) || [];
    const uniqueImages = [...new Set(imageMatches)];

    if (uniqueImages.length > 0) {
      console.log(
        chalk.blue("⬥"),
        ` Found ${uniqueImages.length} unique images. Downloading...`,
      );
      let downloaded = 0;
      let skipped = 0;
      ...
      for (const imgPath of uniqueImages) {
        ...
        const localPath = join(
          localContentPath,
          "images",
          imgPath.replace("/content/images/", ""),
        );
        ...
        process.stdout.write(
          `\r${chalk.dim("┃")} Progress: ${downloaded + skipped}/${uniqueImages.length} (Downloaded: ${downloaded}, Skipped: ${skipped})`,
        );
      }
    ...
    console.log(
      chalk.white(
        ` 1. Open your local Ghost Admin: ${targetInstance.url}/ghost/`,
      ),
    );
    console.log(chalk.white(` 2. Go to Settings > Labs > Import content.`));
    console.log(
      chalk.white(` 3. Upload the exported file: ${basename(exportFilePath)}`),
    );
    console.log(
      chalk.dim(
